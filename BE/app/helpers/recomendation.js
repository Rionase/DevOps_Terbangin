...
let CalculateTicketSimilarity = (ticketToCompare, userTicketHistory) => {
  ...
  userTicketHistory.forEach((item) => {
    ...
    if (
      item["departure_city"] == ticketToCompare["departure_city"] &&
      item["departure_country"] == ticketToCompare["departure_country"]
    )
      point++;
    if (
      item["arrival_city"] == ticketToCompare["arrival_city"] &&
      item["arrival_country"] == ticketToCompare["arrival_country"]
    )
      point++;
...
exports.CalculatePredictionPreference = (
  myId,
  allUser,
  allTicket,
  matriksInteraksi,
  ...
  let totalCosine =
    consineSimilarity.reduce((a, b) => Math.abs(a) + Math.abs(b), 0) -
    Math.abs(consineSimilarity[myUserIndex]);
  ...
  allTicket.forEach((ticket, ticketIndex) => {
    ...
    allUser.forEach((user, userIndex) => {
      if (userIndex != myUserIndex) {
        let kontribusi =
          consineSimilarity[userIndex] *
          (matriksInteraksi[userIndex][ticketIndex] -
            rataRataNilaiInteraksi[userIndex]);
    ...
    let predictionPreference =
      rataRataNilaiInteraksi[myUserIndex] + hasil_pembagian;
    result.push({
...
exports.exportAll = async (
  ...
  // Tentukan path export
  let exportDir = "export";
  let exportPath = `${exportDir}/${email}.xlsx`;

  // Periksa apakah direktori ada, jika tidak, buat direktori baru
  ...
  let headerX_matriksInteraksi = allTicket.map(
    (item) => `${item.flight_code} - ${item.class}`
  );
  let headerY_matriksInteraksi = allUser.map((item) => `${item.email}`);
  writeExcel(
    worksheet1,
    headerX_matriksInteraksi,
    headerY_matriksInteraksi,
    matriksInteraksi
  );
  ...
  // Sheet 3: Prediction Preference
  const worksheet3 = workbook.addWorksheet("Prediction Preference");
  let headerX_preference = allTicket.map(
    (item) => `${item.flight_code} - ${item.class}`
  );
  let headerY_preference = [email];
  let predictionContent = SortPredictionPreferenceExcelData(
    predictionPreference,
    allTicket
  );
  writeExcel(worksheet3, headerX_preference, headerY_preference, [
    predictionContent,
  ]);

  // Simpan file Excel ke path
...
function SortPredictionPreferenceExcelData(predictionPreference, allTicket) {
  const mergedData = predictionPreference.map((preference) => {
    const matchingTicket = allTicket.find(
      (ticket) =>
        ticket.flights_id === preference.flight_id &&
        ticket.class === preference.flight_class
    );
