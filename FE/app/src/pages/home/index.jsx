...
const DestinationFavorit = ({ isFullScreen }) => {
  ...
  const navigateToFindTicket = (flight) => {
    navigate("/find-ticket", {
      state: {
        ...
        iataCodeDeparture: flight.departure_iataCode,
        arrival: flight.arrival_city,
        iataCodeArrival: flight.arrival_iataCode,
        departureDate: moment
          .tz(flight.departure_time, flight.departure_timezone)
  ...
  useEffect(() => {
    let fetchFlight = async () => {
      let fetchFlightData = await getFlightRecomendation(page)
      setFlights(fetchFlightData)
    }
    fetchFlight();
  }, [page]);
  ...
  return (
    <Container>
      ...
      <Row style={{ margin: isFullScreen ? "0px 120px" : "0px 0px" }}>
        {flights.map((flight) => (
          <Col key={`${flight.id} - ${flight.flight_class}`} md={3} sm={6} xs={6} className="mb-3">
            <Card
            ...
            >
              ...
              <Card.Img
                key={flight.id}
                src={`/Foto Airport/${flight.picture}`}
                alt="End Airport"
                // loading="lazy"
                style={{
                  width: "100%",
